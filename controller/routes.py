...
def select():
    try:
        ...
        if data:
            dictionary=[{"id":t.id, "name":t.name, "address":t.address, 
                         "email":t.email} for t in data]
...
def delete():
    id=request.args["id"]
    if not id:
        logging.warning("QUERY PARAMATER ID DOESN'T EXIST DELETE")
        return {"message":"must be exists query paramater id"}, 400
    try:
...
def update():
    ...
    try:
        if use_cases_client.update(Client(**data)):
            ...
            return {"message":"CLIENT DOESNT EXISTS"},404
    except Exception as ex:
